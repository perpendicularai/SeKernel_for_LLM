...
- ```
  ...
  data = kernel.template(prompt=prompt, plugins=plugins.defaultPlugin(), context=search_prompt or context="" # Where no context is provided, and so you may assume the AI assistant to not have any awareness of information of events that took place after the date until which it's training data is up until) # See plugins.py module for more plugins
